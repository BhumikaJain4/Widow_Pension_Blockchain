...
contract IPFSVerifier {
    ...
    event DocumentAnchored(uint256 indexed applicationId, string ipfsCID, bytes32 sha256Hash, string docType, uint256 timestamp);
    event VerificationResult(uint256 indexed applicationId, bytes32 claimedHash, bool isValid, address verifier);
    ...
    function anchorDocument(
        uint256 applicationId, string calldata ipfsCID,
        bytes32 sha256Hash, string calldata docType
        ...
        documents[applicationId].push(DocumentRecord(ipfsCID, sha256Hash, docType, block.timestamp, true));
        cidToAppId[ipfsCID] = applicationId;
        emit DocumentAnchored(applicationId, ipfsCID, sha256Hash, docType, block.timestamp);
    }

    function verifyDocument(
        uint256 applicationId, uint256 docIndex, bytes32 claimedHash
    ...
    function getDocumentManifest(uint256 applicationId) external view returns (DocumentRecord[] memory) {
        return documents[applicationId];
    }

    function getDocument(uint256 applicationId, uint256 docIndex) external view returns (DocumentRecord memory) {
        require(docIndex < documents[applicationId].length, "IPFSVerifier: out of bounds");
